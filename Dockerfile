...
# Ensure Python can import top-level package `DataHandling` (for DataHandling.wsgi, app, etc.)
ENV PYTHONPATH=/app
...
# Use the module path `DataHandling.wsgi:application`.
CMD ["sh", "-c", "waitress-serve --host=0.0.0.0 --port=${PORT:-5000} DataHandling.wsgi:application"]
