...
# Default model to fall back to if auto-detect of "latest" fails
DEFAULT_MODEL_FILENAME = os.environ.get("DEFAULT_MODEL_FILENAME", "model_20260210_172825.pkl")


def initialize_model_and_features():
    """
    Initialize the in-memory model and feature metadata.

    - Try to load the most recently modified .pkl in Config.MODEL_PATH
    - If that fails or no .pkl exists, fall back to DEFAULT_MODEL_FILENAME if present
    - Load processed X_train.pkl to populate feature_columns and feature_medians
    """
    global current_model, model_metadata, feature_columns, feature_medians

    try:
        model_dir = Config.MODEL_PATH
        print(f"[INIT] Looking for models in: {model_dir}")
        ...
        if os.path.exists(model_dir):
            ...
            # 1) Try latest by mtime
            if pkl_files:
                latest = max(pkl_files, key=os.path.getmtime)
                try:
                    print(f"[INIT] Attempting to load latest model: {latest}")
                    current_model = load_pickle(latest)
                    model_metadata.update({
                        'model_path': latest,
                        'loaded_at': datetime.now().isoformat(),
                        'strategy': 'latest_by_mtime',
                    })
                    print("[INIT] Latest model loaded into memory.")
                except Exception as e:
                    print(f"[INIT] Failed to load latest model {latest}: {e}")

            # 2) Fallback to a specific default model if nothing loaded
            if current_model is None and DEFAULT_MODEL_FILENAME:
                fallback_path = os.path.join(model_dir, DEFAULT_MODEL_FILENAME)
                if os.path.exists(fallback_path):
                    try:
                        print(f"[INIT] Attempting to load default model: {fallback_path}")
                        current_model = load_pickle(fallback_path)
                        model_metadata.update({
                            'model_path': fallback_path,
                            'loaded_at': datetime.now().isoformat(),
                            'strategy': 'default_filename',
                        })
                        print("[INIT] Default model loaded into memory.")
                    except Exception as e:
                        print(f"[INIT] Failed to load default model {fallback_path}: {e}")
                else:
                    print(f"[INIT] Default model file not found at: {fallback_path}")
        else:
            print(f"[INIT] Model directory does not exist: {model_dir}")

        if current_model is None:
            print(
                "[INIT] No model loaded at startup. "
                "Please ensure a .pkl exists in Config.MODEL_PATH "
                "or use the /api/model/load endpoint."
            )
        ...
        try:
            ...
            print(f"[INIT] Looking for processed features at: {processed_x_path}")
            if os.path.exists(processed_x_path):
                ...
                print(f"[INIT] Loaded processed features: {len(feature_columns)} columns")
            else:
                feature_columns = None
                feature_medians = {}
                print(f"[INIT] Processed X_train not found at: {processed_x_path}")
        except Exception as e:
            feature_columns = None
            feature_medians = {}
            print(f"[INIT] Failed to load processed X_train for medians: {e}")
    except Exception as e:
        print(f"[INIT] Error during startup model/feature initialization: {e}")
...
# Initialize model and feature metadata on module import as well,
# so WSGI servers (e.g. Waitress on Render) also get a loaded model.
